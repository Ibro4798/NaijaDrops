<#
Fix-DashboardProfileModal.ps1

ROOT CAUSE FOUND: the business name / phone fields from the last batch were
added to src/app/profile/page.jsx - but nothing in the actual vendor UI links
to /profile. The hamburger menu's "Profile" button, and the auto-opening
first-login prompt, both open a DIFFERENT component defined inline inside
src/app/dashboard/page.jsx (the "Identity Profile" modal you saw in your
screenshot) - which only ever had Full Name + Photo.

This patches that actual modal to add Business Name and Phone Number, wires
them to load existing values and save correctly (users.phone,
vendors.business_name via upsert on user_id), and fixes a related bug found
while in there: the modal was being pre-filled with only the vendor's FIRST
name (used for the dashboard greeting), so re-opening "Profile" to edit and
hitting Save would silently truncate the saved name down to first-name-only
every time. Now tracked separately so the greeting still shows first name
but the edit modal shows and saves the full name.
...
$TargetPath = "src\app\dashboard\page.jsx"
...
if (-not (Test-Path $TargetPath)) {
    Write-Host "ERROR: $TargetPath not found. Run from repo root." -ForegroundColor Red
    exit 1
}
...
Write-Utf8Bom -Path $TargetPath -Content $newContent
Write-Host "Patched: $TargetPath" -ForegroundColor Green

Write-Host "`nReview and ship:" -ForegroundColor Cyan
Write-Host "  git diff"
Write-Host "  git add $TargetPath"
Write-Host "  git commit -m `"Add business name + phone to the actual profile modal vendors use, fix name-truncation bug`""
Write-Host "  git push"
