...
import { Loader2, MapPin, Package, CheckCircle2, Clock } from 'lucide-react';
    ...
    async function load() {
      ...
      if (user) {
        const { data: authedOrder } = await supabase
          .from('orders')
          .select('*, riders(current_lat, current_lng, users(full_name))')
          .eq('id', orderId)
          .single();
        if (authedOrder) {
      ...
      <div className="min-h-screen bg-charcoal-950 px-6 py-12 flex justify-center">
        <div className="w-full max-w-md bg-charcoal-900 border border-white/10 rounded-[2rem] p-8 space-y-6">
          <div className="flex flex-col items-center text-center gap-2">
            ...
            <p className="text-white font-black text-xl font-outfit">Delivered</p>
            <p className="text-charcoal-400 text-xs">{new Date(order.updated_at).toLocaleString()}</p>
          </div>
          <div className="space-y-3 border-t border-white/10 pt-6">
            <div className="flex justify-between text-sm"><span className="text-charcoal-400">Item</span><span className="text-white font-bold">{order.item_description}</span></div>
            <div className="flex justify-between text-sm"><span className="text-charcoal-400">From</span><span className="text-white font-bold">{order.pickup_name}</span></div>
            <div className="flex justify-between text-sm"><span className="text-charcoal-400">To</span><span className="text-white font-bold">{order.dropoff_name}</span></div>
