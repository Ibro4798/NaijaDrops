<#
    NaijaDrops - apply-fixes.ps1  (consolidated, encoding root-cause fixed)
    ------------------------------------------------------------------
    FIX (this version, the real root cause): every previous version of
    this script had its non-ASCII characters (Naira sign, em-dashes, the
    stray embedded BOM text) silently garbled the moment Windows
    PowerShell 5.1 opened it - because the .ps1 FILE ITSELF had no
    byte-order-mark, so PowerShell 5.1 read it using the system's ANSI
    codepage instead of UTF-8, misinterpreting every multi-byte character
    before a single line of the script even ran. That's what kept
    reintroducing "ï»¿" mojibake and mangled ₦ symbols no matter how many
    times the OUTPUT-writing logic was fixed - the corruption was already
    baked into what PowerShell had read into memory. This file now starts
    with a real UTF-8 BOM so PowerShell 5.1 reads it correctly from the
    first byte. (The actual project source files it writes still get
    plain UTF-8 with NO byte-order-mark, which is what Next.js expects -
    only this .ps1 wrapper itself needed one.)
    ...
    IMPORTANT: delete any old apply-fixes.ps1 (and any "(1)", "(2)" etc.
    duplicate copies, and their .bak files) from your project folder
    before running this one, so there's no chance of re-running a stale,
    corrupted version by accident.

    WHAT'S IN THIS PASS:
      - Pay Now button on Active Orders (src/app/vendor/active-orders/page.jsx)
      - Location: GPS-only, no IP fallback, 15s window (src/utils/geolocation.js)
      ...
      - Payment page wording/theme + already-paid guard + replace nav (src/app/payment/page.jsx)
      - Public track API - extra fields for receipts/map (src/app/api/track/[orderId]/route.js)
      - Receipt share AS AN IMAGE, not a link (src/components/ui/ReceiptShareButton.jsx)
      - Receipt redesign - no commission, perforated edge, download (src/app/receipt/[orderId]/page.jsx)
      - Stepper dedup, live rider-location realtime, bulletproof share link (src/app/tracking/[orderId]/page.jsx)
      - Map - multi-marker bounds fit + route line (src/components/MapCanvas.jsx)
      - Vendor order-status toast notifications (src/components/OrderStatusNotificationListener.jsx)
      ...
      - Receipt display name field (src/app/profile/page.jsx)
      ...
      - Visual status stepper + duplicate step collapsed (src/components/ui/OrderStatusStepper.jsx)
      - Lock price negotiation once payment is confirmed (src/components/OrderChat.jsx)
      - New: canvas-based receipt image renderer (no new dependency) (src/utils/receiptImage.js)
    ...
    DATABASE (already applied directly to your live Supabase project -
    nothing to run for these):
      - receipt_display_name column on users
      - service_role grants across all public tables (was missing
        project-wide)
      - orders/riders/messages/wallet_transactions added to the
        supabase_realtime publication (was EMPTY - this is why nothing
        auto-updated without a manual refresh, anywhere in the app)
      - authenticated role granted access to wallet_transactions (the
        admin finance page couldn't read pending rider payout requests
        at all before this)
#>
...
# which silently adds a BOM. $NewContent also has any stray leading BOM
# character stripped defensively.
...
# ------------------------------------------------------------------
# Location: GPS-only, no IP fallback, 15s window
# ------------------------------------------------------------------
$content_1 = @'
/**
...
Set-WholeFile -RelativePath "src\utils\geolocation.js" -NewContent $content_1 -Label "Location: GPS-only, no IP fallback, 15s window"
...
# ------------------------------------------------------------------
# Payment page wording/theme + already-paid guard + replace nav
# ------------------------------------------------------------------
$content_4 = @'
"use client";
...
Set-WholeFile -RelativePath "src\app\payment\page.jsx" -NewContent $content_4 -Label "Payment page wording/theme + already-paid guard + replace nav"
...
# ------------------------------------------------------------------
# Receipt share AS AN IMAGE, not a link
# ------------------------------------------------------------------
$content_6 = @'
"use client";
...
Set-WholeFile -RelativePath "src\components\ui\ReceiptShareButton.jsx" -NewContent $content_6 -Label "Receipt share AS AN IMAGE, not a link"

# ------------------------------------------------------------------
# Receipt redesign - no commission, perforated edge, download
# ------------------------------------------------------------------
$content_7 = @'
"use client";
...
import { createClient } from '@/utils/supabase/client';
import { CheckCircle2, Package, Printer, Star, ArrowRight, Sparkles, Download, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-charcoal-950 print:bg-white">
      ...
      <div className="max-w-md mx-auto px-6 -mt-14 pb-16 print:mt-0 print:px-0">
        ...
        <div className="bg-charcoal-900 border border-white/10 border-t-0 rounded-b-[2rem] p-8 space-y-6 shadow-premium print:bg-white print:border-none print:shadow-none print:text-black print:rounded-none">

          {(senderName || senderLogo) && (
            <div className="flex flex-col items-center text-center gap-3 pb-4 border-b border-dashed border-white/10 print:border-charcoal-300">
              {senderLogo ? (
          ...
          <div className="space-y-3">
            ...
            {/* Platform commission removed - a customer/vendor-facing receipt
                should reflect what was paid, not NaijaDrops' internal cut.
                Vendors can see commission breakdowns in their earnings/
                history views instead. */}
            <div className="flex justify-between text-sm pt-3 border-t border-dashed border-white/10 print:border-charcoal-300"><span className="text-charcoal-400 font-bold">Total Paid</span><span className="text-emerald-400 font-black text-lg print:text-black">₦{total.toLocaleString()}</span></div>
          </div>

          <div className="flex flex-col gap-3 pt-2 print:hidden">
            <div className="grid grid-cols-3 gap-3">
              <ReceiptShareButton receiptData={receiptData} />
              <button
...
Set-WholeFile -RelativePath "src\app\receipt\[orderId]\page.jsx" -NewContent $content_7 -Label "Receipt redesign - no commission, perforated edge, download"

# ------------------------------------------------------------------
# Stepper dedup, live rider-location realtime, bulletproof share link
# ------------------------------------------------------------------
$content_8 = @'
"use client";
...
Set-WholeFile -RelativePath "src\app\tracking\[orderId]\page.jsx" -NewContent $content_8 -Label "Stepper dedup, live rider-location realtime, bulletproof share link"
...
# ------------------------------------------------------------------
# Vendor order-status toast notifications
# ------------------------------------------------------------------
$content_10 = @'
"use client";
...
Set-WholeFile -RelativePath "src\components\OrderStatusNotificationListener.jsx" -NewContent $content_10 -Label "Vendor order-status toast notifications"
...
Set-WholeFile -RelativePath "src\app\profile\page.jsx" -NewContent $content_13 -Label "Receipt display name field"
...
# ------------------------------------------------------------------
# Visual status stepper + duplicate step collapsed
# ------------------------------------------------------------------
$content_17 = @'
"use client";
...
export default function OrderStatusStepper({ steps, currentStatus }) {
  // FIX: 'pending' and 'looking_for_driver' both display as "Finding a
  // rider" - showing them as two separate steps duplicated the same label
  // back to back. Collapse them into a single visual step.
  const displaySteps = [];
  for (const s of steps) {
    if ((s === 'pending' || s === 'looking_for_driver') && displaySteps.includes('looking_for_driver')) continue;
    displaySteps.push(s);
  }
  const effectiveStatus = currentStatus === 'pending' ? 'looking_for_driver' : currentStatus;
  const currentIndex = displaySteps.indexOf(effectiveStatus);
...
Set-WholeFile -RelativePath "src\components\ui\OrderStatusStepper.jsx" -NewContent $content_17 -Label "Visual status stepper + duplicate step collapsed"

# ------------------------------------------------------------------
# Lock price negotiation once payment is confirmed
# ------------------------------------------------------------------
$content_18 = @'
"use client";
...
import { useState, useEffect, useRef } from 'react';
import { createClient } from '@/utils/supabase/client';
import { Send, X, AlertCircle, DollarSign, Check, ChevronDown, Mic, Play, Square, Lock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
...
export default function OrderChat({ orderId, currentUserId, onClose, isReadOnly = false, onPriceUpdated }) {
  ...
  useEffect(() => {
    const fetchOrderAndMessages = async () => {
      ...
      const { data: orderData } = await supabase
        .from('orders')
        .select('agreed_price, status, payment_status, vendor_id, rider_id')
        .eq('id', orderId)
        .single();
  ...
  // FIX: negotiation used to stay open for the entire order lifecycle up
  // until delivered/cancelled - including well after payment_status
  // flipped to 'paid'. The agreed_price at that point is literally what
  // was charged through Paystack; changing it afterward via chat only
  // updates the database record, not the actual transaction, silently
  // creating a mismatch between what the customer paid and what the order
  // says they agreed to. Negotiation is only meaningful (and only allowed)
  // before payment is confirmed - available to both the vendor and the
  // rider equally, since neither role is restricted here beyond that.
  const canNegotiatePrice = order && order.status !== 'delivered' && order.status !== 'cancelled' && order.payment_status !== 'paid';
  const priceLockedByPayment = order && order.payment_status === 'paid' && order.status !== 'delivered' && order.status !== 'cancelled';
        ...
        <div className="flex items-center gap-3">
          ...
          {order && (
            <button
              ...
              title={canNegotiatePrice ? "Tap to negotiate price" : priceLockedByPayment ? "Payment already confirmed - price is locked" : "Order complete"}
            >
...
Set-WholeFile -RelativePath "src\components\OrderChat.jsx" -NewContent $content_18 -Label "Lock price negotiation once payment is confirmed"

# ------------------------------------------------------------------
# New: canvas-based receipt image renderer (no new dependency)
# ------------------------------------------------------------------
$content_19 = @'
/**
...
Set-WholeFile -RelativePath "src\utils\receiptImage.js" -NewContent $content_19 -Label "New: canvas-based receipt image renderer (no new dependency)"
