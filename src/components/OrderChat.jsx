...
export default function OrderChat({ orderId, currentUserId, onClose, isReadOnly = false }) {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (error) {
      ...
      alert(`Chat Error: ${error?.message || JSON.stringify(error)}`);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-charcoal-900 animate-slide-up sm:absolute sm:rounded-t-3xl sm:shadow-2xl sm:top-auto sm:h-[80vh]">
      ...
      {isReadOnly && (
        <div className="p-6 bg-charcoal-800/50 border-t border-charcoal-800 text-center">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">Order Completed • Chat Closed</p>
        </div>
      )}
    </div>
  );
