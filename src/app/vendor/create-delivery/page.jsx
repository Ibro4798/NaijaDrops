...
export default function CreateDelivery() {
  ...
  const [linkResolveError, setLinkResolveError] = useState({ pickup: null, dropoff: null });
  ...
  const handleSearchChange = (val, slot) => {
    ...
    setLinkResolveError(prev => ({ ...prev, [slot]: null }));
    ...
    // FIX: pasting a Google Maps / WhatsApp share link into this box used
    // to be handed straight to Mapbox/OSM as a plain text search string -
    // no geocoder can match a raw URL against a place name, so it just
    // silently returned zero results. Short links (maps.app.goo.gl) are
    // even less likely to resolve this way since the real coordinates
    // only exist after following the redirect. This detects a map link
    // and routes it through the same resolver already used in the
    // send-package flow (follows short-link redirects server-side, and
    // falls back to geocoding the place name/address for business-listing
    // links with no raw coordinates at all), then opens the confirm-pin
    // map directly on the resolved point - the same next step as picking
    // a normal search suggestion.
    const maybeUrl = extractFirstUrl(val);
    if (maybeUrl && isAllowedHost(maybeUrl)) {
      ...
      (async () => {
        try {
          ...
          if (latestSearchRef.current[slot] !== val) return; // input changed since - stale response

          if (res.ok && data?.lat && data?.lng) {
            let name = data.matchedName || `${data.lat.toFixed(5)}, ${data.lng.toFixed(5)}`;
            try {
              const resolvedName = await reverseGeocodeMapbox(data.lat, data.lng, process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN);
              if (resolvedName) name = resolvedName;
            } catch {
              // reverse-geocode is a nicety for the label only - the pin
              // itself is already correct, so this never blocks proceeding
            }
            setSearchInputs(prev => ({ ...prev, [slot]: '' }));
            handleSelectSuggestion({ name, lat: data.lat, lng: data.lng }, slot);
          } else {
            setLinkResolveError(prev => ({
              ...prev,
              [slot]: data?.message || "Couldn't resolve that link. Open it in Maps, tap the pin, and choose \"Share location\" - or just search by name instead."
            }));
          }
        } catch {
          setLinkResolveError(prev => ({ ...prev, [slot]: 'Connection failed while resolving that link. Check your network and try again.' }));
        } finally {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // empty with no way to tell "still typing" apart from "this
        // actually failed" - now says so directly.
        setLinkResolveError(prev => ({ ...prev, [slot]: 'Search failed - check your connection and try again.' }));
      } finally {
        setIsSearching(prev => ({ ...prev, [slot]: false }));
      }
    }, 400);
