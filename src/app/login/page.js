...
import { Package, Mail, Lock, User, ArrowRight, Eye, EyeOff, AlertCircle } from "lucide-react";
...
export default function Login() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // If successful, create profile entry (Wait, Supabase handles this via trigger usually,
        // but for now let's just alert them to verify email if needed or redirect)
        if (
          data.user &&
  ...
  return (
    <div className="min-h-screen bg-charcoal-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-extrabold text-charcoal-900 tracking-tight">
          {isLogin ? "Sign in to NaijaDrops" : "Create your account"}
        </h2>
